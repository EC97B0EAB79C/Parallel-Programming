...
int main()
{
	...
	// set variable
	if (readDouble("../test/n1000/m.double", m) != N) {
	...
	if (readDouble("../test/n1000/x.double", pos) != N) {
		fprintf(stderr, "Err: Can not read x.double\n");
		return -1;
	}
	if (readDouble("../test/n1000/y.double", pos + N) != N) {
		fprintf(stderr, "Err: Can not read y.double\n");
		return -1;
	}
	if (readDouble("../test/n1000/z.double", pos + N * 2) != N) {
